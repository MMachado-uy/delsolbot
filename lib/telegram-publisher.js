...
// IPv4-only keep-alive agent. DigitalOcean droplets frequently have a dead IPv6
// route to api.telegram.org; Node's happy-eyeballs (autoSelectFamily) stalls on
// the broken AAAA path and surfaces as AggregateError [ETIMEDOUT] at connect
// time. Pinning family:4 sidesteps the broken address family entirely.
const telegramAgent = new https.Agent({ keepAlive: true, family: 4 });

// Per-request timeout (ms). Previously unset (axios default 0 = no timeout), so a
// stalled socket could hang indefinitely. Generous enough for a ~50 MB part on a
// well-connected host; override via env if the droplet's link is slower.
const UPLOAD_TIMEOUT_MS = Number(process.env.TELEGRAM_TIMEOUT_MS) || 120000;
...
const MAX_SEND_ATTEMPTS = Number(process.env.TELEGRAM_SEND_RETRIES) || 3;
const RETRY_BASE_DELAY_MS = Number(process.env.TELEGRAM_RETRY_DELAY_MS) || 2000;
