...
jest.mock('./helpers', () => {
    ...
    return {
        ...
        // Resolve immediately so in-attempt retry backoff doesn't slow tests.
...
describe('lib/telegram-publisher', () => {
    ...
    describe('sendEpisodeToChannel (exercised via outer function) — request contract', () => {
        ...
        it('pins family:4 and a finite timeout on the upload request', async () => {
            axios.mockResolvedValue({ data: makeTelegramSuccessResponse() });
            splitEpisode.mockResolvedValue(['/mocked/path']);
    ...
    describe('in-attempt network retry (hardening)', () => {
        it('retries a transient ETIMEDOUT and succeeds, recording a single success row', async () => {
            const transient = Object.assign(new Error('connect ETIMEDOUT'), { code: 'ETIMEDOUT' });
            axios
                .mockRejectedValueOnce(transient)
                .mockResolvedValueOnce({ data: makeTelegramSuccessResponse({ file_id: 'AFTER-RETRY' }) });
            splitEpisode.mockResolvedValue(['/mocked/path']);
            ...
            expect(db.registerUpload.mock.calls[0][0].fileId).toBe('AFTER-RETRY');
        });
        ...
        it('retries 429 (rate limit) responses', async () => {
            ...
            axios
                ...
                .mockResolvedValueOnce({ data: makeTelegramSuccessResponse() });
            splitEpisode.mockResolvedValue(['/mocked/path']);
        ...
        it('does NOT retry a permanent 400 Bad Request — fails fast after one attempt', async () => {
            const badRequest = Object.assign(new Error('Bad Request'), {
                response: { status: 400, body: { description: 'chat not found' } }
            });
            axios.mockRejectedValue(badRequest);
            splitEpisode.mockResolvedValue(['/mocked/path']);

            const result = await sendToTelegram(makeItem(), 'Podcast');

            expect(result).toBe(false);
            expect(axios).toHaveBeenCalledTimes(1); // no retry on a 4xx
            expect(db.registerUpload.mock.calls[0][0].exito).toBe(false);
        });

        it('gives up after MAX_SEND_ATTEMPTS transient failures and records a failure row', async () => {
            const transient = Object.assign(new Error('socket reset'), { code: 'ECONNRESET' });
            axios.mockRejectedValue(transient);
            splitEpisode.mockResolvedValue(['/mocked/path']);

            const result = await sendToTelegram(makeItem(), 'Podcast');

            expect(result).toBe(false);
            expect(axios).toHaveBeenCalledTimes(3); // default MAX_SEND_ATTEMPTS
            expect(db.registerUpload.mock.calls[0][0].exito).toBe(false);
        });

        it('builds a fresh read stream for each retry (a stream is single-use)', async () => {
            const transient = Object.assign(new Error('connect ETIMEDOUT'), { code: 'ETIMEDOUT' });
            const streams = [makeReadStream(), makeReadStream()];
            let i = 0;
            createReadStreamSpy.mockImplementation(() => streams[i++]);
            axios
                .mockRejectedValueOnce(transient)
                .mockResolvedValueOnce({ data: makeTelegramSuccessResponse() });
            splitEpisode.mockResolvedValue(['/mocked/path']);
            ...
            expect(streams[0].destroy).toHaveBeenCalled(); // first (failed) stream cleaned up
        });
    });

    describe('caption safety (HTML escaping + length cap)', () => {
        it('HTML-escapes the title and content in the stored caption', async () => {
            axios.mockResolvedValue({ data: makeTelegramSuccessResponse() });
            splitEpisode.mockResolvedValue(['/mocked/path']);

            await sendToTelegram(makeItem({ title: 'A & B', content: 'x <i>y</i> & z' }), 'Podcast');

            const { caption } = db.registerUpload.mock.calls[0][0];
            expect(caption).toBe('<b>A &amp; B</b>\nx &lt;i&gt;y&lt;/i&gt; &amp; z');
            expect(caption).not.toContain('<i>');
        });

        it('truncates an over-long description so the caption stays within Telegram limits', async () => {
            axios.mockResolvedValue({ data: makeTelegramSuccessResponse() });
            splitEpisode.mockResolvedValue(['/mocked/path']);

            await sendToTelegram(makeItem({ title: 'T', content: 'x'.repeat(5000) }), 'Podcast');

            const { caption } = db.registerUpload.mock.calls[0][0];
            const rendered = caption.replace(/<\/?b>/gu, '');
            expect(rendered.length).toBeLessThanOrEqual(1024);
            expect(caption).toContain('…');
        });
    });

    describe('Telegram error description capture (error.response.data)', () => {
        it('records the Telegram error description in obs (not just the generic axios message)', async () => {
            const apiError = Object.assign(new Error('Request failed with status code 400'), {
                response: { status: 400, data: { description: 'Bad Request: message caption is too long' } }
            });
            ...
            expect(result).toBe(false);
            expect(axios).toHaveBeenCalledTimes(1); // 400 is non-transient → no retry
            expect(db.registerUpload.mock.calls[0][0].obs).toContain('caption is too long');
        });
    });
