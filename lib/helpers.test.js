...
describe('lib/helpers', () => {
    ...
    describe('escapeHtml', () => {
        it('escapes &, < and > so text is safe in Telegram HTML mode', () => {
            expect(escapeHtml('<b> & </b>')).toBe('&lt;b&gt; &amp; &lt;/b&gt;');
        });

        it('escapes & FIRST so introduced entities are not re-escaped', () => {
            // Order matters: a naive < then & pass would turn &lt; into &amp;lt;.
    ...
    describe('buildCaption', () => {
        it('wraps the title in <b> and escapes title and content', () => {
            expect(buildCaption('Title & <x>', 'Body <y> & z'))
                .toBe('<b>Title &amp; &lt;x&gt;</b>\nBody &lt;y&gt; &amp; z');
        });

        it('keeps short captions intact (no truncation, no ellipsis)', () => {
            const msg = buildCaption('Ep', 'short description');
            expect(msg).toBe('<b>Ep</b>\nshort description');
            expect(msg).not.toContain('…');
        });

        it('truncates long content so the rendered caption stays under the 1024 limit', () => {
            const longContent = 'x'.repeat(5000);
            const msg = buildCaption('Title', longContent);
            // Rendered length excludes the <b></b> tags but counts the visible text.
            const rendered = msg.replace(/<\/?b>/gu, '');
            expect(rendered.length).toBeLessThanOrEqual(1024);
            expect(msg).toContain('…');
        });

        it('handles null/undefined title and content without throwing', () => {
            expect(buildCaption(undefined, null)).toBe('<b></b>\n');
        });
    });

    describe('envInt', () => {
        const ORIGINAL = process.env.TEST_ENV_INT;

        afterEach(() => {
            if (ORIGINAL === undefined) delete process.env.TEST_ENV_INT;
            else process.env.TEST_ENV_INT = ORIGINAL;
        });

        it('parses a valid non-negative integer', () => {
            process.env.TEST_ENV_INT = '5';
            expect(envInt('TEST_ENV_INT', 99)).toBe(5);
        });

        it('honors a configured 0 (does not fall back like Number(x) || default)', () => {
            process.env.TEST_ENV_INT = '0';
            expect(envInt('TEST_ENV_INT', 99)).toBe(0);
        });

        it('falls back when unset or non-numeric', () => {
            delete process.env.TEST_ENV_INT;
            expect(envInt('TEST_ENV_INT', 99)).toBe(99);
            process.env.TEST_ENV_INT = 'abc';
            expect(envInt('TEST_ENV_INT', 99)).toBe(99);
        });
    });

    describe('getIdFromItem error handling', () => {
        it('throws a clear error when item.link is missing', () => {
            expect(() => getIdFromItem({})).toThrow(/item\.link is missing/u);
        });

        it('throws a clear error when item.link is not a string', () => {
            expect(() => getIdFromItem({ link: 12345 })).toThrow(/item\.link is number/u);
        });
    });
