...
describe('lib/process-item', () => {
    ...
    describe('retry path — prior attempts failed on this channel (Defect #2 fix)', () => {
        ...
        it('counts a multi-part failure as ONE attempt (per-part rows do not exhaust budget)', async () => {
            // A single failed multi-part upload inserts one row per part on the
            // same tick. The budget must count attempts, not part rows, or a
            // 3-part episode would be abandoned after one transient failure.
            db.getPodcastById.mockResolvedValue([
                makeStoredRow({ archivo: '123-1', channel: '@current', pudo_subir: 0, file_id: '' }),
                makeStoredRow({ archivo: '123-2', channel: '@current', pudo_subir: 0, file_id: '' }),
                makeStoredRow({ archivo: '123-3', channel: '@current', pudo_subir: 0, file_id: '' })
            ]);
        ...
        it('abandons a multi-part episode only after MAX attempts of every part failing', async () => {
            // Three failed attempts × 2 parts = 6 rows, but only 3 attempts.
            const failedPart = (n) => makeStoredRow({
                archivo: `123-${n}`, channel: '@current', pudo_subir: 0, file_id: ''
            });
            db.getPodcastById.mockResolvedValue([
                failedPart(1), failedPart(1), failedPart(1),
                failedPart(2), failedPart(2), failedPart(2)
            ]);
